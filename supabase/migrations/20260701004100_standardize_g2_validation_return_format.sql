-- Standardize validate_briefing_for_distribution to return { is_valid, issues_count, issues }
-- and fix distribute_project to use the correct key.
-- Allows same-day projects (end_date >= start_date).
...
BEGIN
  ...
  -- Content checks
  v_val := v_briefing->>'objetivo';
  IF v_val IS NULL OR length(trim(v_val)) = 0 THEN
    v_issues := v_issues || jsonb_build_object('rule', 'Objetivo', 'message', 'O objetivo do projeto não foi preenchido.');
    v_is_valid := false;
  END IF;

  v_val := v_briefing->>'publico_alvo';
  IF v_val IS NULL OR length(trim(v_val)) = 0 THEN
    v_issues := v_issues || jsonb_build_object('rule', 'Público-alvo', 'message', 'O público-alvo não foi preenchido.');
    v_is_valid := false;
  END IF;

  v_val := v_briefing->>'canais';
  IF v_val IS NULL OR length(trim(v_val)) = 0 THEN
    v_issues := v_issues || jsonb_build_object('rule', 'Canais', 'message', 'Os canais não foram preenchidos.');
    v_is_valid := false;
  END IF;

  v_val := v_briefing->>'budget';
  IF v_val IS NULL OR length(trim(v_val)) = 0 THEN
    v_issues := v_issues || jsonb_build_object('rule', 'Budget', 'message', 'O orçamento não foi preenchido.');
    v_is_valid := false;
  END IF;

  v_val := v_briefing->>'restricoes';
  IF v_val IS NULL OR length(trim(v_val)) = 0 THEN
    v_issues := v_issues || jsonb_build_object('rule', 'Restrições', 'message', 'As restrições não foram preenchidas.');
    v_is_valid := false;
  END IF;

  v_val := v_briefing->>'referencias';
  IF v_val IS NULL OR length(trim(v_val)) = 0 THEN
    v_issues := v_issues || jsonb_build_object('rule', 'Referências', 'message', 'As referências não foram preenchidas.');
    v_is_valid := false;
  END IF;
  ...
  -- Date validation: allow same-day projects (end_date >= start_date)
  -- Only fail if end_date < start_date
...
-- Fix distribute_project to use is_valid key (was using is_valid but function returned "valid")
