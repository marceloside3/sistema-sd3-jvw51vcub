-- Universal REUNIÃO DE PASSAGEM DE BRIEFING field for all areas
-- Add reuniao_passagem_briefing to criacao and producao, and make it universal for ALL areas
...
BEGIN
  ...
  LOOP
    CASE v_area.code
      ...
      WHEN 'criacao' THEN
        v_required_fields := ARRAY[
          'reuniao_passagem_briefing', 'contexto_projeto', 'especificacoes_tecnicas',
          'infos_comerciais', 'budget_qtde_prazos', 'redacao',
          'assets_marca_guidelines', 'referencias'
        ];
      WHEN 'producao' THEN
        v_required_fields := ARRAY[
          'reuniao_passagem_briefing', 'contexto_projeto', 'especificacoes_tecnicas',
          'infos_comerciais', 'budget_qtde_prazos'
        ];
      WHEN 'social' THEN
        v_required_fields := ARRAY[
          'reuniao_passagem_briefing', 'contexto_projeto', 'contexto_mercadologico',
      ...
      ELSE
        v_required_fields := ARRAY['reuniao_passagem_briefing'];
    END CASE;
